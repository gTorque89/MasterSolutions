...
function agregarProductoAFavorito (id) {
...
function eliminarItemDeFavorito (id) {
    ...
    const productoSeleccionado = listaFavoritos.findIndex((producto) => producto.id === id);
    favoritos.splice(productoSeleccionado, 1);
    const totalCarrito = carritoAcumulado.reduce((acumulador, producto) => acumulador - producto.precio, 0);
    document.getElementById('total-favoritos').innerHTML = favoritos.length + " - $" + totalCarrito;
...
function generarCardsFavoritos () {
    cardsCarrito()
